...
export default function SignupPage() {
  ...
  async function handleSubmit(e) {
    ...
    setLoading(true);
    try {
      await signUpWithRollNumber(rollNumber, password);
      navigate("/dashboard");
    } catch (err) {
      const message = err.message.replace("Firebase: ", "");
      if (message.includes("email-already-in-use")) {
        setError("This roll number is already registered. Try logging in instead.");
      } else {
        setError(message);
      }
    }
      ...
      <div className="relative w-full max-w-5xl grid lg:grid-cols-2 gap-10 items-center">
        <div className="hidden lg:block">
          ...
          <h2 className="text-4xl font-bold mb-4 leading-tight text-gray-900">
            Create your <span className="text-emerald-600">Resistance</span> account
          </h2>
          <p className="text-gray-500 mb-8 max-w-md">
            One-time signup with your roll number. No real name, no email — just you and your resistance.
          </p>
        ...
        <div className="w-full max-w-md mx-auto lg:mx-0">
          ...
          <div className="bg-white p-6 sm:p-8 lg:p-10 rounded-2xl border border-gray-100 shadow-[0_1px_3px_rgba(0,0,0,0.04),0_20px_40px_-12px_rgba(0,0,0,0.1)]">
            ...
            <p className="text-sm text-gray-400 text-center mb-6">Create your account to get started</p>
